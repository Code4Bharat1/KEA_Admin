...
export default function SeminarDetailsPage() {
  ...
  const fetchSeminar = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/seminars/${id}`);
      // 🔥 backend returns seminar object directly
      setSeminar(data);
    } catch (err) {
      console.error('Fetch seminar error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-3 py-1 rounded text-xs font-medium ${styles[status] || 'bg-gray-100 text-gray-700'}`}>
        {status}
      </span>
    );
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600 mt-6">
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4" />
              {seminar.date && new Date(seminar.date).toLocaleDateString()}
            </div>
